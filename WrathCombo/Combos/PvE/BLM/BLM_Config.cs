            ...
            switch (preset)
            {
                ...
                case CustomComboPreset.BLM_Blizzard1to3:
                    DrawRadioButton(BLM_B1to3,
                        $"Replaces {Blizzard.ActionName()}", $"在未處於靈極冰3層時將{Blizzard.ActionName()}替換為{Blizzard3.ActionName()}", 0);

                    DrawRadioButton(BLM_B1to3,
                        $"Replaces {Blizzard3.ActionName()}", $"在靈極冰3層時將{Blizzard3.ActionName()}替換為{Blizzard.ActionName()}", 1);
                    break;

                case CustomComboPreset.BLM_Fire1to3:
                    DrawRadioButton(BLM_F1to3,
                        $"Replaces {Fire.ActionName()}", $"在未處於星極火3層或未戰鬥時將{Fire.ActionName()}替換為{Fire3.ActionName()}", 0);

                    DrawRadioButton(BLM_F1to3,
                        $"Replaces {Fire3.ActionName()}", $"在星極火3層時將{Fire3.ActionName()}替換為{Fire.ActionName()}", 1);
                    break;
            }
        }
