...
namespace WrathCombo.Window.Tabs
{
    internal class Settings : ConfigWindow
    {
        internal new static void Draw()
        {
            ...
            {
                ImGui.Text("This tab allows you to customise your options for Wrath Combo.".Loc());

                #region UI Options

                ImGuiEx.Spacing(new Vector2(0, 20));
                ImGuiEx.TextUnderlined("Main UI Options".Loc());

                #region SubCombos

                var hideChildren = Service.Configuration.HideChildren;

                if (ImGui.Checkbox("Hide SubCombo Options".Loc(), ref hideChildren))
                {
                    Service.Configuration.HideChildren = hideChildren;
                    Service.Configuration.Save();
                }

                ImGuiComponents.HelpMarker("Hides the sub-options of disabled features.".Loc());
                ...
                if (ImGui.Checkbox("Hide Conflicted Combos".Loc(), ref hideConflicting))
                {
                    Service.Configuration.HideConflictedCombos = hideConflicting;
                    Service.Configuration.Save();
                }

                ImGuiComponents.HelpMarker("Hides any combos that conflict with others you have selected.".Loc());
                ...
                if (ImGui.Checkbox("Shorten Server Info Bar Text".Loc(), ref shortDTRText))
                {
                ...
                ImGuiComponents.HelpMarker((
                    ...
                    "\nThis option will make the number of active Auto-Mode combos not show").Loc()
                );
                ...
                if (ImGui.Checkbox("Hide Message of the Day".Loc(), ref motd))
                {
                    Service.Configuration.HideMessageOfTheDay = motd;
                    Service.Configuration.Save();
                }

                ImGuiComponents.HelpMarker("Disables the Message of the Day message in your chat when you login.".Loc());
                ...
                ImGui.SameLine();
                ImGui.TextColored(ImGuiColors.DalamudGrey, "(Only used by ?? currently)".Loc(CustomComboFunctions.JobIDs.JobIDToName(33)));
                ...
                if (ImGui.Checkbox("Open Wrath to the PvE Features tab".Loc(), ref
                ...
                ImGuiComponents.HelpMarker(("When you open Wrath with `/wrath`, it will open to the PvE Features tab, instead of the last tab you were on." +
                                           "\nSame as always using the `/wrath pve` command to open Wrath.").Loc());

                #endregion

                #region Open to PvP

                if (ImGui.Checkbox("Open Wrath to the PvP Features tab in PvP areas".Loc(), ref
                ...
                ImGuiComponents.HelpMarker(("Same as above, when you open Wrath with `/wrath`, it will open to the PvP Features tab, instead of the last tab you were on, when in a PvP area." +
                                           "\nSimilar to using the `/wrath pvp` command to open Wrath.").Loc());

                ImGui.SameLine();
                ImGui.TextColored(ImGuiColors.DalamudGrey, "(Will override the option above)".Loc());

                #endregion

                #region Open to Current Job

                if (ImGui.Checkbox("Open PvE Features UI to Current Job on Opening".Loc(), ref Service.Configuration.OpenToCurrentJob))
                    Service.Configuration.Save();

                ImGuiComponents.HelpMarker("When you open Wrath's UI, if your last tab was PvE, it will automatically open to the job you are currently playing.".Loc());

                #endregion

                #region Open to Current Job on Switching

                if (ImGui.Checkbox("Open PvE Features UI to Current Job on Switching Jobs".Loc(), ref Service.Configuration.OpenToCurrentJobOnSwitch))
                    Service.Configuration.Save();

                ImGuiComponents.HelpMarker("When you switch jobs, it will automatically switch the PvE Features tab to the job you are currently playing.".Loc());
                ...
                ImGuiEx.Spacing(new Vector2(0, 20));
                ImGuiEx.TextUnderlined("Rotation Behavior Options".Loc());

                #region Spells while Moving

                if (ImGui.Checkbox("Block spells if moving".Loc(), ref Service.Configuration.BlockSpellOnMove))
                    Service.Configuration.Save();
                ...
                if (ImGui.Checkbox("Action Replacing".Loc(), ref Service.Configuration.ActionChanging))
                    Service.Configuration.Save();
                ...
                if (Service.Configuration.ActionChanging) {
                    ImGui.Indent();
                    if (ImGui.Checkbox("Performance Mode".Loc(), ref Service.Configuration.PerformanceMode))
                        Service.Configuration.Save();
                ...
                if (ImGui.Checkbox("Queued Action Suppression".Loc(), ref Service.Configuration.SuppressQueuedActions))
                    Service.Configuration.Save();
                ...
                ImGui.TextColored(ImGuiColors.DalamudGrey, "read more:".Loc());
                ...
                ImGuiComponents.HelpMarker(("With this enabled, whenever you queue an action that is not the same as the button you are pressing, it will disable every other button's feature from running. This resolves a number of issues where incorrect actions are performed due to how the game processes queued actions, however the visual experience on your hotbars is degraded. This is not recommended to be disabled, however if you feel uncomfortable with hotbar icons changing quickly this is one way to resolve it (or use Performance Mode) but be aware that this may introduce unintended side effects to combos if you have a lot enabled for a job." +
                    ...
                    "\n\nOur workaround for this is to disable all other actions being replaced if they don't match the queued action, which this setting controls.").Loc());
                ...
                var len = new[]
                    {
                        "milliseconds".Loc(), "seconds".Loc(), "yalms".Loc(),
                        "%% of cast".Loc(), "oGCDs".Loc(),
                    }
                    .Max(unit => ImGui.CalcTextSize(unit).X);
                ...
                ImGui.Text("milliseconds".Loc());
                ImGui.SameLine(pos);
                ImGui.Text("   -   Action Updater Throttle".Loc());
                ...
                ImGui.SameLine();
                ImGui.Text("seconds".Loc());

                ImGui.SameLine(pos);

                ImGui.Text("   -   Movement Check Delay".Loc());
                ...
                ImGui.Text("   -   Opener Failure Timeout".Loc());
                ...
                ImGui.SameLine();
                ImGui.Text("yalms".Loc());
                ImGui.SameLine(pos);

                ImGui.Text("   -   Melee Distance Offset".Loc());
                ...
                ImGui.SameLine( pos);
                ImGui.Text("   -   Interrupt Delay".Loc());
                ...
                ImGui.SameLine();
                ImGui.Text("oGCDs".Loc());

                ImGui.SameLine(pos);

                ImGui.Text("   -   Maximum number of Weaves".Loc());
                ...
                if (ImGui.Checkbox("Retarget (Single Target) Healing Actions".Loc(), ref retargetHealingActions))
                {
                    Service.Configuration.RetargetHealingActionsToStack =
                ...
                if (ImGui.Checkbox("Add Out of Party NPCs to Retargeting".Loc(), ref addNpcs))
                {
                ...
                ImGui.TextUnformatted("Current Heal Stack:".Loc());
                ...
                ImGuiComponents.HelpMarker((
                    ...
                    "If the 'Retarget Healing Actions' option is enabled, that target is also the one that healing actions will be targeted onto (even when the action does not first check the HP of that target, like the combo's Replaced Action, for example).").Loc());
                ...
                else
                {
                    if (Service.Configuration.UseUIMouseoverOverridesInDefaultHealStack)
                        healStackText += "UI-MouseOver Target".Loc() + nextStackItemMarker;
                    if (Service.Configuration.UseFieldMouseoverOverridesInDefaultHealStack)
                        healStackText += "Field-MouseOver Target".Loc() + nextStackItemMarker;
                    healStackText += "Soft Target".Loc() + nextStackItemMarker;
                    healStackText += "Hard Target".Loc() + nextStackItemMarker;
                    if (Service.Configuration.UseFocusTargetOverrideInDefaultHealStack)
                        healStackText += "Focus Target".Loc() + nextStackItemMarker;
                    if (Service.Configuration.UseLowestHPOverrideInDefaultHealStack)
                        healStackText += "Lowest HP% Ally".Loc() + nextStackItemMarker;
                    healStackText += "Self".Loc();
                }
                ImGuiEx.Spacing(new Vector2(10, 0));
                ...
                var labelText = "Heal Stack Customization Options".Loc();
                ...
                if (_unCollapsed)
                {
                    ...
                    if (ImGui.Checkbox("Add UI MouseOver to the Default Healing Stack".Loc(), ref useUIMouseoverOverridesInDefaultHealStack))
                    {
                        Service.Configuration.UseUIMouseoverOverridesInDefaultHealStack =
                    ...
                    if (ImGui.Checkbox("Add Field MouseOver to the Default Healing Stack".Loc(), ref useFieldMouseoverOverridesInDefaultHealStack))
                    {
                        Service.Configuration.UseFieldMouseoverOverridesInDefaultHealStack =
                    ...
                    if (ImGui.Checkbox("Add Focus Target to the Default Healing Stack".Loc(), ref useFocusTargetOverrideInDefaultHealStack))
                    {
                        Service.Configuration.UseFocusTargetOverrideInDefaultHealStack =
                    ...
                    if (ImGui.Checkbox("Add Lowest HP% Ally to the Default Healing Stack".Loc(), ref useLowestHPOverrideInDefaultHealStack))
                    {
                        Service.Configuration.UseLowestHPOverrideInDefaultHealStack =
                    ...
                    if (useLowestHPOverrideInDefaultHealStack)
                    {
                        ...
                        ImGuiEx.Text(ImGuiColors.DalamudYellow, "This should be used with the 'Retarget Healing Actions' setting above!".Loc());
                    }
                    if (useCusHealStack) ImGui.EndDisabled();
                    ...
                    if (ImGui.Checkbox("Use a Custom Heal Stack Instead".Loc(), ref useCustomHealStack))
                    {
                    ...
                    if (Service.Configuration.UseCustomHealStack)
                    {
                        ...
                        UserConfig.DrawCustomStackManager(
                            ...
                            "Default: Focus Target > Hard Target > Self").Loc()
                        );
                        ImGui.Unindent();
                    }
                ...
                ImGui.TextUnformatted("Current Raise Stack:".Loc());
                ...
                ImGuiComponents.HelpMarker((
                    ...
                    "or under each caster that has a Raise.").Loc());
                ...
                UserConfig.DrawCustomStackManager(
                    ...
                    "Default: Any Healer > Any Tank > Any Raiser > Any Dead Party Member").Loc(),
                    true
                );
                ImGui.TextDisabled("(all targets are checked for rezz-ability)".Loc());
                ImGui.Unindent();
                ...
                ImGuiEx.Spacing(new Vector2(0, 20));
                ImGuiEx.TextUnderlined("Troubleshooting / Analysis Options".Loc());

                #region Combat Log

                bool showCombatLog = Service.Configuration.EnabledOutputLog;

                if (ImGui.Checkbox("Output Log to Chat".Loc(), ref showCombatLog))
                {
                    Service.Configuration.EnabledOutputLog = showCombatLog;
                    Service.Configuration.Save();
                }

                ImGuiComponents.HelpMarker("Every time you use an action, the plugin will print it to the chat.".Loc());
                #endregion

                #region Opener Log

                if (ImGui.Checkbox("Output opener status to chat".Loc(), ref Service.Configuration.OutputOpenerLogs))
                    Service.Configuration.Save();

                ImGuiComponents.HelpMarker("Every time your class's opener is ready, fails, or finishes as expected, it will print to the chat.".Loc());
                #endregion

                #region Debug File

                if (ImGui.Button("Create Debug File".Loc()))
                {
                    if (Player.Available)
